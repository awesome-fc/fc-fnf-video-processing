...
aliyun ram UpdateRole --RoleName aliyunfcgeneratedrole-$REGION-video-demo --NewAssumeRolePolicyDocument "$(cat ./assume-role-policy.json)"
...
if [ "$ACTION" == "create" ]; then
  ...
  aliyun ros CreateStack --RegionId $REGION --StackName $STACK_NAME --TimeoutInMinutes 10 \
    ...
    --Parameters.2.ParameterValue acs:ram::$ACCOUNT_ID:role/aliyunfcgeneratedrole-$REGION-video-demo \
    --Parameters.3.ParameterKey FlowNameFC \
    --Parameters.3.ParameterValue $FLOW_NAME
